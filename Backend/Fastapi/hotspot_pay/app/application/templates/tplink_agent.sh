#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════
# HOTSPOTPAY — AGENT TP-LINK (AUTO-GENERE)
# ═══════════════════════════════════════════════════════
...
#   2. chmod +x hotspotpay-tplink-agent.sh
#   3. ./hotspotpay-tplink-agent.sh &
# ═══════════════════════════════════════════════════════
...
executer_api() {
    local action="$1" user="$2" pass="$3" profile="$4"
    curl -sk "http://${ROUTEUR_IP}:${ROUTEUR_PORT}/cgi-bin/hotspot.cgi" \
        -d "action=${action}&username=${user}&password=${pass}&profile=${profile}"
}

creer_utilisateur() {
    local user="$1" pass="$2" profile="${3:-default}"
    executer_api "add" "$user" "$pass" "$profile"
    log "CREE: $user (profile=$profile)"
}

supprimer_utilisateur() {
    local user="$1"
    executer_api "delete" "$user" "" ""
    log "SUPPRIME: $user"
}
...
# BOUCLE PRINCIPALE
# ═══════════════════════════════════════════════════════
log "═══ Agent HotspotPay TP-LINK demarre (hotspot=$HOTSPOT_ID) ═══"
