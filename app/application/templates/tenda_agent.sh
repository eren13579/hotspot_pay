#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════
# HOTSPOTPAY — AGENT TENDA (AUTO-GENERE)
# ═══════════════════════════════════════════════════════
...
#   2. chmod +x hotspotpay-tenda-agent.sh
#   3. ./hotspotpay-tenda-agent.sh &
# ═══════════════════════════════════════════════════════
...
creer_utilisateur() {
    ...
    curl -sk "http://${ROUTEUR_IP}:${ROUTEUR_PORT}/goform/HotspotUserAdd" \
        -H "Cookie: ${TENDA_COOKIE}" \
        -d "username=${user}&password=${pass}&profile=${profile}"
    log "CREE: $user (profile=$profile)"
}

supprimer_utilisateur() {
    local user="$1"
    curl -sk "http://${ROUTEUR_IP}:${ROUTEUR_PORT}/goform/HotspotUserDel" \
        -H "Cookie: ${TENDA_COOKIE}" \
        -d "username=${user}"
    log "SUPPRIME: $user"
}
...
kick_session() {
    local mac="$1"
    curl -sk "http://${ROUTEUR_IP}:${ROUTEUR_PORT}/goform/HotspotKick" \
        -H "Cookie: ${TENDA_COOKIE}" \
        -d "mac=${mac}"
    log "KICK: $mac"
}
...
# BOUCLE PRINCIPALE
# ═══════════════════════════════════════════════════════
log "═══ Agent HotspotPay Tenda demarre (hotspot=$HOTSPOT_ID) ═══"
